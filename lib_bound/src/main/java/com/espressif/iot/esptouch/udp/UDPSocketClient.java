...
public class UDPSocketClient {
  ...
  public void sendData(byte[][] data, int offset, int count,
    ...
    for (int i = offset; !mIsStop && i < offset + count; i++) {
      ...
      try {
        String temp = "乐鑫每20ms发送一次报文\r\n发送UDP目标IP： " + targetHostName + ":" + targetPort + "\r\n" + "data[" + i + " +].length = " + data[i].length;
...
//        Log.i(TAG, "data[" + i + " +].length = " + data[i].length);
  ...
  public void sendData(byte[] data, int count,
    ...
    for (int i = 0; i < count; i++) {
      try {
        DatagramPacket localDatagramPacket = new DatagramPacket(data, data.length, InetAddress.getByName(targetHostName), targetPort);
        this.mSocket.send(localDatagramPacket);
      } catch (UnknownHostException e) {
        e.printStackTrace();
        if (__IEsptouchTask.DEBUG) {
