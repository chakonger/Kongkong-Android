...
public class UDPSocketClient {
     ...
     * 用于新岸线发送
     */
    public void sendDateForNewPort(byte[] configBytes) {
        int k;
        int l;
        byte[] srcbs = new byte[1024];
        System.arraycopy(configBytes, 0, srcbs, 0, configBytes.length);
        k = 1;
        for (int i = 0; i < configBytes.length; i++) {
            if (mIsStop)
                break;
            l = (configBytes[i] + 256) % 256;
            l = l == 0 ? 129 : l;
            l = k == 1 ? (156 + l) : (156 - l);
            k = 1 - k;
            byte[] bs = new byte[l];
            ...
            for (int j = 0; j < 20; j++) {
                ...
                try {
                    DatagramPacket localDatagramPacket = new DatagramPacket(
                            bs, bs.length,
                            InetAddress.getByName("239.1.2.110"), 60001);
                    this.mSocket.send(localDatagramPacket);
                } catch (UnknownHostException e) {
                    if (__IEsptouchTask.DEBUG) {
                        Log.e(TAG, "sendData(): UnknownHostException");
